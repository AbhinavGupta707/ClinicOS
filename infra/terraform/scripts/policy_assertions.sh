...
malware_scanner="$root_dir/modules/malware-scanner"
storage="$root_dir/modules/storage/main.tf"
...
require 'module "malware_scanner"' "$platform" "platform must compose the isolated GuardDuty scanner module"
require 'CLINIC_OS_MEDIA_STORAGE_PROVIDER.*aws_s3' "$platform" "production API must use the durable S3 media provider"
require 'CLINIC_OS_MEDIA_INSPECTION_PROVIDER.*guardduty_s3' "$platform" "production API must use GuardDuty evidence"
require 'CLINIC_OS_MEDIA_BINDING_SECRET' "$platform" "media request bindings must use runtime secret material"
require 'lambda:InvokeFunction' "$platform" "API must be able to invoke only the isolated evidence signer"
require 'kms:Verify' "$platform" "API must verify asymmetric malware evidence"
reject 'kms:Sign' "$platform" "the API/platform task policy must never mint malware evidence"
require 'malware-protection-plan.guardduty.amazonaws.com' "$malware_scanner" "only the GuardDuty malware service may assume the scan role"
require 'GuardDutyMalwareScanStatus' "$malware_scanner" "GuardDuty scan status must be the provider-owned verdict source"
require 'kms:Sign' "$malware_scanner" "the isolated evidence signer must have the bounded signing action"
require 'aws_lambda_alias' "$malware_scanner" "API invocation must bind an immutable published Lambda alias"
require 'I_ACKNOWLEDGE_GUARDDUTY_S3_AND_TAGGING_COSTS' "$malware_scanner" "scanner activation must require explicit spend acknowledgement"
require 'OnlyGuardDutyMayWriteManagedMalwareStatus' "$storage" "storage must prevent application principals from forging GuardDuty status"
