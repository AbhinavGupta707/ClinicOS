  ...
  for (const contract of contracts) {
    ...
    if (contract.ownerLane === "security_privacy") {
      assert.equal(
        ...
        true,
        `${contract.key} must track the merged Security/Privacy API route`
      );
    }
    if (contract.ownerLane === "fhir_abdm") {
      assert.equal(
        contract.liveImplemented,
        false,
        `${contract.key} remains projection-package evidence until a live FHIR API route exists`
      );
    }
...
function assertPerformanceSmoke(performanceSmoke, knownActorKeys) {
  ...
  for (const endpoint of performanceSmoke.deferredEndpoints) {
    ...
    assert.match(endpoint.reason, /canonical|route activation|deferred/i);
  }
}
