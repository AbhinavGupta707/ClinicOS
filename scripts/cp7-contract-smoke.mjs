...
const TOKEN_ENV_BY_ACTOR = {
  assistant: "CLINICOS_CP7_ASSISTANT_TOKEN",
  owner: "CLINICOS_CP7_OWNER_TOKEN"
};
...
function parseArgs(argv) {
  const options = {
    authMode: process.env.CLINICOS_CP7_AUTH_MODE ?? "bearer",
    baseUrl: process.env.CLINICOS_CP7_API_BASE_URL ?? "",
    dryRun: false
  };
...
export function buildCp7SmokePlan(scenario) {
...
function printDryRun(plan) {
  console.log("CP7 API smoke dry run plan:");
...
async function runLiveSmoke(scenario, plan, options) {
  assert.ok(options.baseUrl, "Set --base-url or CLINICOS_CP7_API_BASE_URL for live smoke.");
  const executable = plan.flowRequests.filter((request) => request.liveImplemented);

  if (executable.length === 0) {
    console.log(
      "No CP7 flow requests are marked liveImplemented in this lane fixture; dry-run contract is the expected lane evidence until backend routes merge."
    );
    return;
  }
...
async function main() {
  ...
  const plan = buildCp7SmokePlan(scenario);
