...
| CP5       |                12 | pricebook, plans, procedure evidence, invoices, payments, instructions, Razorpay webhook |
...
| WhatsApp inbound/status callback       | provider unregistered   | `/v1/webhooks/whatsapp/*`                         | No signed Meta callback route/provider registration exists.               |
...
Razorpay webhook code is registered, but official provider callback registration/sandbox evidence is
still a CP15 activation gate. Registration in this inventory does not imply provider activation.
