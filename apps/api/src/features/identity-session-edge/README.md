...
The master integration must instantiate `IdentitySessionEdgeGuard` with a canonical `/realms/<slug>` Keycloak issuer, `clinic-os-api` audience, accepted `clinic-os-web-bff` and mobile client IDs, a maximum five-minute access-token lifetime, the production `__Host-` session-cookie name, a distributed durable revocation store and a distributed durable required security-audit outbox. Startup readiness must call `guard.readiness()` and fail when either dependency is unavailable. Production issuers reject userinfo, query, fragment, ports and noncanonical realm paths; local HTTP is loopback-only.
