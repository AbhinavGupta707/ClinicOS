  ...
  return {
    api: api
      ? {
          ...
          requestId: readString(api, ["requestId", "request_id", "correlationId", "correlation_id"]) ?? undefined,
...
function getRequestId(response: Response, payload: unknown) {
  if (isRecord(payload)) {
    const topLevel = readString(payload, ["request_id", "requestId", "correlation_id", "correlationId"]);
    const error = readNestedRecord(payload, ["error"]);
    const nested = error ? readString(error, ["request_id", "requestId", "correlation_id", "correlationId"]) : null;
