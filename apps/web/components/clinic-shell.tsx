...
export function ClinicShell({ initialSurfaceId }: ClinicShellProps) {
  ...
  if (meState.status === "unauthenticated" || meState.status === "unavailable") {
    return <AuthStatusPanel onRetry={refreshMe} problem={meState.problem} status={meState.status} />;
  }
  ...
  return (
    <div className="clinic-shell">
      <aside className={navOpen ? "clinic-nav clinic-nav--open" : "clinic-nav"} aria-label="ClinicOS navigation">
        <div className="nav-brand">
          <Link className="brand-mark" href="/" onClick={() => setActiveSurfaceId("today")}>
            <span className="brand-mark__symbol" aria-hidden="true">C</span>
            <span>
          ...
          {visibleSurfaces.map((surface) => {
            ...
            return (
              ...
              >
                ...
                {surface.availability !== "active" ? <span className="nav-status">Later</span> : null}
              </Link>
            );
          })}
        </nav>
