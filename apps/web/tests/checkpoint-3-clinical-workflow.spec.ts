...
test.describe("Checkpoint 3 doctor clinical workflow", () => {
  ...
  test("captures intake and consent state, signs note, amends note, and signs prescription", async ({
    ...
    await page.goto("/surface/clinical?scenario=cp3-intake-consent-encounter");
    ...
    await page.screenshot({
      fullPage: true,
      path: "/private/tmp/clinicos-cp3-web-doctor-desktop.png"
    });
  });
  ...
  test("mobile clinical workflow shell has no horizontal overflow", async ({ page }) => {
    ...
    await page.screenshot({
      fullPage: true,
      path: "/private/tmp/clinicos-cp3-web-mobile-390.png"
    });
  });
});
