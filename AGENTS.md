...
- Keep `main` at the last verified checkpoint or explicitly owner-approved implementation baseline. Integrate worker commits on `codex/integration/checkpoint-N`, reconcile shared files once, run every currently executable gate, record any external evidence deferral without overclaiming, then promote to `main` and run post-promotion checks.
