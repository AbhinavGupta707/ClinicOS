...
test("CP9 dry-run route contracts cover export, audit, FHIR, break-glass, and retention", async () => {
  ...
  assert.ok(
    plan.routeContracts
      .filter((request) => request.ownerLane === "security_privacy")
      .every((request) => request.liveImplemented === true)
  );
  assert.ok(
    plan.routeContracts
      .filter((request) => request.ownerLane === "fhir_abdm")
      .every((request) => request.liveImplemented === false)
  );
