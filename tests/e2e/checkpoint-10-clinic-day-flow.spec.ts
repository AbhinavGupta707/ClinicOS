...
test.describe("Checkpoint 10 release-candidate clinic-day smoke", () => {
  ...
  test("owner sees aggregate dashboard and compliance remains honestly unavailable", async ({
    ...
    await expectSurface(
      page,
      "/surface/pilot-readiness?scenario=cp10",
      "cp10-pilot-readiness-workspace"
    );
    await expect(page.getByTestId("cp10-go-live-status")).toContainText("Blocked");
    await expect(page.getByTestId("cp10-live-gaps")).toContainText("ABDM");
    await expect(page.getByTestId("cp10-pilot-readiness-workspace")).not.toContainText(
      "Pilot ready"
    );
    ...
    await expect(
      page.getByLabel("Surface state").getByText("Registered unavailable")
    ).toBeVisible();
